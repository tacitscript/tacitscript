...
export default {
	...
	description: <div>
		<p>The <a href="#pipe">. (pipe)</a> operation takes a prefix operator to the left and right. The resulting expression passes a value through the left operator, then feeds the result through the right operator.</p>
		<div className="code-block">{getOperationExamples([
			["halfAddOne", "/2.+1"],
			["calculation", "halfAddOne4", "equals 3"],
		])}</div>
		<p>The <a href="#dot">.</a> and <a href="#comma">,</a> operators break <i>left-associativity</i> having <b>lowest</b> precedence (the only exceptions to this rule.) Therefore, parentheses are not required around <span className="code">+1</span> above.</p>
	</div>,
	exercise: {
		question: <div>
