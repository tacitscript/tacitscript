...
export default {
	...
	exercise: {
		question: <div>
			...
			<div>Define the operator <b>toCelsius</b> such that:</div>
		</div>,
		...
		tests: [
			...
			{description: testValue => <span><b>toCelsius</b>{`${testValue} equals ${toDecimalPlaces(toCelsius(testValue), 4)}`}</span>, condition: ({solution, testValue}) => {
				if (typeof solution !== "function") return false;

				try {
					return Math.abs(solution(testValue) - toCelsius(testValue)) < 1E-10;
				} catch (ex) {
					return false;
				}
			}},
