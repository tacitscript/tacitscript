...
export default {
	...
	description: <div>
		<p><a href="#pipe">. (pipe)</a> is an example of a <i>higher order</i> operation; one that takes other operators as operands.</p>
		<p>The <a href="#dollar">$</a> operator represents a group of operations that <i>join</i> elements together.</p>
		<p><a href="#reduce">$ (reduce)</a> applies the left-hand infix operator between each element of the array on the right.</p>
...
*/`,
		...
		hint2: "pipe reduce",
		tests: [
			{description: "chain(-32 /9 *5) gives a prefix operator (fToC)", condition: ({solution}) => typeof (solution === "function") && (solution.length === 1)},
