...
export default {
	...
	name: "Higher Order Operations",
	operations: <React.Fragment><a href="#reduce">($) reduce</a></React.Fragment>,
	description: <div>
		<p><a href="#pipe">(.) pipe</a> is an example of a <i>higher order</i> operation; one that takes other operations as arguments.</p>
		<p>The <a href="#dollar">dollar ($)</a> operator represents a group of operations that <i>join</i> elements together.</p>
		<p><a href="#reduce">($) reduce</a> inserts the left-hand infix operation between each element of the array on the right.</p>
		<div className="code-block">{getOperationExamples([
			["sum", "+$(3 4 5)", "equals 12"],
		])}</div>
		<p>A variation is to provide a starting value alongside the operation on the left.</p>
		<div className="code-block">{getOperationExamples([
			["sumOnTwo", "(+ 2)$(3 4 5)", "equals 14"],
		])}</div>
		<p>As well as building values this way, we can construct algorithms from arrays of operations.</p>
	</div>,
	exercise: {
		question: <div>
			<div>Define the higher order operation <b>chain</b> such that:</div>
		</div>,
		getJs: ({def}) => `/*ts
chain		${def}
...
*/`,
		...
		hint2: "pipe reduce",
		tests: [
			{description: "chain(-32 /9 *5) gives a prefix operation (fToC)", condition: ({solution}) => typeof (solution === "function") && (solution.length === 1)},
