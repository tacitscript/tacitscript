...
export default {
	...
	exercise: {
		question: <span>Define the operator <b>isEvenNT</b> that takes an integer and returns whether the number is even:</span>,
		getJs: ({def}) => `const solution = /*ts ${def} */;`,
		getHtml: details => <div className="single-line name-expression">
			<div className="name">isEvenNT</div>
		...
		tests: [
			...R.times(() => ({
				description: testData => <span><b>isEvenNT</b>{`${testData} equals ${ts.toString(isEven(testData))}`}</span>,
				condition: ({solution, testValue}) => isEven(testValue) === solution(testValue),
			}), 2),
			{description: () => <span><b>isEvenNT</b> uses the <a href="#percent">percent (%)</a> operator</span>, condition: ({es6}) => es6.includes("ts.percent")},
		],
	},
};