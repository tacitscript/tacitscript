	...
	describe("Operators", () => {
		...
		describe("dollar ($)", () => {
			...
			*/
			...
			it('AVA					append				(1 2)$3=(1 2 3)', () => expect(append).eql([1, 2, 3]));
		});
		...
		describe("question (?)", () => {
			...
			*/
			...
			it('(VV)AA				filter				<5?(4 9 2 7 3)', () => expect(filter).eql([4, 2, 3]));
		});
		...
		describe("plus (+)", () => {
			...
			*/
			...
			it("AAA					arrayConcat			(1 2 3)+(4 5 6)=(1 2 3 4 5 6)", () => expect(arrayConcat).eql([1, 2, 3, 4, 5, 6]));
			it('DDD					merge				\(("a" 1) ("b" 2))+(\(("b" 3))=\(("a" 1) ("b" 3))', () => expect(merge).eql({a: 1, b: 3}));
		});
